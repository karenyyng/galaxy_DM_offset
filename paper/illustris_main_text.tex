...
those peak is at $|\Delta s| = \sigma$. 
...
\subsubsection{Comparison of the offset results to other studies}
...
inferring cosmological parameters from the cluster mass abundance 
relationship.
In the following, we show that cosmological simulations generally show 
the BCGs has the tightest offsets from the DM peaks.
Some cosmological simulations show that misidentifying the BCG can reproduce
the scatter found in observational studies of BCG-DM peak offset. 
We then discuss the  is why the BCG or luminosity peak  
do not have as tight of an agreement with some (weak) lensing studies,
and why they may not maximize the stacked lensing signal.

what other summary statistic 
to use as a proxy of the DM peak when the BCG of a cluster cannot be
identified. Here we compare and contrast the properties of second best proxy to the DM
peak than the BCG. Then we discuss whether any of the galaxy summary statistic
can minimize stacking error.

To establish the baseline of the tightest $\Delta s$, we first discuss and 
compare the $\Delta s_{\rm BCG}$ constraints.   
Observations were performed with a much larger FOV from the Subaru Suprime-Cam 
The bulk of the distribution of $\Delta s_{\rm BCG}$ is below 3 kpc. 
This is consistent with only having smoothing noise of the DM map affecting the
identification of the BCG. This offset distribution is found to be much tighter than most 
of the observations. 
The analysis of 25 massive galaxy clusters of the LoCuSS survey by \cite{Oguri2010} 
fitting elliptical NFW
models to the weak lensing data, also show a two
component distribution for $\Delta s_{\rm BCG}$, with the 2D Gaussian standard
deviation being 90
$h^{-1}$ kpc that describes the
offset for most clusters, and a long tail that spans around 1 Mpc.
 The most extreme value of $|\Delta s_{\rm
BCG}|$ in our study is 2.3 Mpc for the full samples but it is clearly a
mismatch to a substructure.  

Despite strong pieces of evidence showing that the BCG having the tightest
$\Delta s$, a unique BCG does not always exist for a cluster (or the subcluster).   
...
The luminosity peaks from the Illustris samples show a much tighter offset to the 
...
typical offset magnitude with standard deviation at 50 - 150 kpc from the
...
Although observational constraints may cause luminosity peak not to work as
accurately as  advertised in this paper, 
there is observational evidence that reaffirm that luminosity peaks can
closely trace DM overdensities in real life.
Using 10 000 Sloan Digital Sky Survey (SDSS) clusters in the GMBCG cataloygue, \cite{Zitrin2012}
performed automated strong lensing (SL) analysis to determine a proxy for DM
peak from galaxy data and compared them to the location of BCG. 
Most cluster were observed with a small FOV of around 78 - 110 $h^{-1}$ kpc$^2$.
The richness of the clusters span from several members to 150 members. 
After testing for different degrees of smoothing polynomials carefully and an
assumed power law spatial distribution of galaxies, \cite{Zitrin2012} found a
24-th degree spline interpolation to best fit the galaxy data for a
luminosity map.
They found such close agreement between the densest luminosity peak
(proxy of DM peak) and the BCG. The mode of the distribution of offset
magnitude between the BCG and the luminosity peak is $\sim$ 4.2 $h^{-1}$ kpc, after removing
a second artifact offsets peak at around the size of the FOV.
Using the luminosity peak as a proxy of the DM peak, \cite{Zitrin2012}
were able to recover strong lensing arcs. 
% Despite the removal of outliers, 
% \cite{Zitrin2012} reported a long tail spanning
% $\sim$ 1 $h^{-1}$ Mpc. 
...
Finally, we have estimated the tight distribution of $\Delta s_{\rm BCG}$ and
$\Delta s_{\rm KDE}$, from visualization, 
For the BCG to work well for stacking, there needs to be rotational symmetry
about the BCG. 
we find neither of those galaxy summary statistic can
characterize some of the asymmetrical or elongated 
cluster distribution well for stacking. 
Some clusters with noticeable substructures 
do not fit the spherical or elliptical approximations that are often assumed 
for clusters. 
Both BCG and luminosity peak often land on top of one of the dense subcomponents. 
The weighted centroids,  
which are the proxies for the center of mass, on the other hand, 
often land between dense substructures.
Quoting the mass within a fixed radius may force the mass distribution to conform
to the spherical / elliptical model assumption.  

Our study is complementary to other cosmological simulations in understanding
the effects of assuming different centers for galaxy clusters. 
In particular, some studies using N-body hydrodynamical cosmological
simulations have tried to characterized the 2D distances between the BCG and 
the second most massive galaxies, to see the effects of misidentifications of 
BCGs.  \cite{Johnston2007b} and 
\cite{Hilbert2010} (using the Millenium simulation)  found 
the one sigma level offsets at 380 $h^{-1}$ kpc, and 410 $h^{-1}$ kpc 
respectively. 

\cite{Cui2015}, using 184 galaxy clusters with M$ > 10^{14}$ M$_\odot$ in a
cosmological simulation suite powered by GADGET2 and GADGET3, 
...
also identified the maximum smoothed particle hydrodynamical (SPH) density peak with various summary location of
the galaxy population of their cluster samples. \cite{Cui2015} found
...
spanning up to several hundred $h^{-1}$ kpc due to disturbed cluster morphology. Our 
...
So far, we have showed and discussed that there are several possible
causes for the offsets, including the summary 
methods for the galaxy population and how substructures affect the different
inference methods. There is one other potential source of major contribution to the
offset. 
\cite{Dietrich2012} ray-traced through the DM substructures in the N-body 
Millennium Run simulation as mock lensing observation for 512 clusters.  
They showed that a combination of shape noise and 
smoothing alone can lead to offsets between the most bound particle and the
lensing peak. The offsets do not simply depend on the smoothing bandwidth, 
but also the number density of the source galaxies that are lensed. 
Even at the source galaxy density of space-based optical data of n = 80
arcmin$^{-1}$, smoothing can cause an offset distribution with the mode at
15 arcsec in the presence of shape noise ($\sim 60$ kpc at z = 0.3). 
Using NFW
halo centers as the DM peak on the Millenium Run simulation data, they showed
that fitting NFW halos can also have comparable distribution of offsets
from the most bound particle (the mode of the distribution at 9 arcsecs
; \citealt{Dietrich2012}). 
While this source of uncertainty is
relatively unimportant in this analysis due to the very high resolution of the
DM particles than sparser source galaxies for lensing, it explains why the uncertainties from the smoothing of the observed
DM convergence maps need to be accounted for during the comparison between the
Illustris results and observations.
...
estimate, but with possibly lower signal-to-noise than the peak. 
...
Staged simulation are valuable, ideal experiments for probing contribution
of possible SIDM to offsets 
where measurement uncertainties are mostly negligible.  
...
(\citealt{Massey2011}, \citealt{Harvey2015}) that may
provide order-of-magnitude estimations, 
the accuracies of their theories are not verified in any SIDM simulation. 
...
The quoted staged SIDM simulations represent some of the maximal offset signal
that can be detected due to SIDM. This is because of the denser assumed halo
...
to the mass density of the infalling halos. These interactions include the
expulsive collisions proposed by \cite{Kahlhoefer14}. Subclusters infalling 
into the main cluster, not only may not produce offset of a comparable
magnitude, it is harder to determine their peaks with high enough accuracies 
due to the lack of strong lensing information. 

Some of the main common takeaways from the listed SIDM simulations are that the
offset levels are observed below 50 kpc regardless of the explored SIDM models. 
