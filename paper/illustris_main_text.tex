...
simulations. We discuss some factors (other than SIDM) that other studies have
shown to affect the observed offsets, including the lensing resolution, the
choice of summary statistic for the member galaxy population and the field of
view. A few observational
studies have shown that bright member galaxies or luminosity peak approximates
the BCG well. 
...
are tighter than most observational studies  
...
Gaussian with a standard deviation of 420 $h^{-1}$ kpc.
...
\cite{Dietrich2012} performed an analogous analysis to \cite{Oguri2010} 
...
(proxy of the BCG) and the lensing peak.  
...
arcmin$^{-1}$, fitting NFW halos and using the center as the DM gave the
mode of the $|\Delta s_{\rm BCG}|$ distribution to be around 9 arcsecs (this is
...
($\sim 90$ kpc and $\sim 400$ kpc at z = 0.3, comparable to \cite{Oguri2010}). 
In addition, \cite{Dietrich2012} showed that smoothing resulted 
in an offset distribution with the mode at
...
relatively unimportant in the Illustris analysis due to the very high resolution 
of the DM particles than sparser source galaxies for lensing, it explains why 
the bootstrapped uncertainties from the observed
...
the galaxy population of a cluster, and show that observations in general agree 
that the luminosity peak is the second best choice.
