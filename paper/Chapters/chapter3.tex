...
There are various settings that these simulations put in place to maximize
...
% When zero impact parameter was initialized for the simulated mergers, 
% Kim et al. (in prep.), using 5.7k or 57k galaxy tracer particles, also show 
% null galaxy-DM offset during most periods of their control staged simulation 
% with $\sigmaSIDM = 0~\centi\meter^2 /$ g. 
Both \cite{Robertson2016} and
\cite{Kahlhoefer14} also showed approximately zero $\offset_{\SIDM}$ throughout their CDM 
simulation runs. 

There are more problems when simulated $\Delta s$ are used to infer
$\offset_{\SIDM}$.  
...
In addition, we will perform a hypothesis test with the galaxy-DM offsets in
...
\begin{figure*}
	...
	Gaussian mixtures look like. A zoomed-in view of the data as shown by the
